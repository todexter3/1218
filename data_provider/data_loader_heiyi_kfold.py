...
class Dataset_regression():
    ...
    def __get_data__(self):
        ...
        columns = ['time', 'ticker', self.pred_task, 'ret_slp', 'ret', 'close_adj', 'high_adj', 'low_adj', 'open_adj',
        ...
        #df[feature_cols] = df.groupby('time')[feature_cols].transform(lambda x: (x - x.mean()) / x.std())
        df[self.pred_task] = df.groupby('time')[self.pred_task].transform(
        ...
        for _, group in grouped:
            ...
            for col, rule in fill_rules.items():
                if col in group.columns:
                    if rule == 'ffill':
                        ...
                        group[col] = group[col].fillna(rule)
            ...
            test = group[
                (group['time'] >= str(self.test_year + '-01-01')) & (
                            group['time'] <= str(self.test_year + '-12-31'))]
        ...
        nowcast_grouped = train_set.groupby('ticker')
        for _, group in nowcast_grouped:
            data = group[
                (group['time'] >= str(self.train_start_year + '-01-01')) & (
                        group['time'] <= str(str(int(self.train_end_year) - 1) + '-12-31'))]
            vali = group[
                (group['time'] >= str(self.train_end_year + '-01-01')) & (
                        group['time'] <= str(self.train_end_year + '-12-31'))]
        ...
        # label_scalerg = RobustScaler(quantile_range=(5, 95))
    ...
    def __read_data__(self):
        ...
        if self.set_type == 0:
            ...
            self.nowcast_ticker = nowcast_data.iloc[:, 1:2]
...
class Dataset_regression_dataset(Dataset):
    def __init__(self, data_x, data_y, tickers, data_stamp, seq_len):
        ...
        for ticker, indices in self.ticker_indices.items():
            num_windows = max(0, (len(indices) - seq_len) + 1)  # 计算有效滑动窗口
            if num_windows > 0:
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.data_length)):
            if index < self.data_length[i]:
                ...
                s_begin = self.start_indices[i - 1] + rel_idx  # ←
                ...
                time_gra = {'ticker': str(self.tickers.iloc[s_end - 1:s_end].values[0]),
                            'time': str(self.data_stamp['time'].iloc[s_end - 1:s_end].values[0])}

                seq_f = self.data_y[s_begin:s_end] 
...
def Dataset_regression_test(args):
    ...
    seq_len = args.size[0]
    test_dataset = Dataset_regression_dataset(dataset.test_set, dataset.test_label, dataset.test_ticker,
